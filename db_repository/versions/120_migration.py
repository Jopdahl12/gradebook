...
assignment = Table('assignment', pre_meta,
    ...
    Column('class_name', VARCHAR(length=80)),
)
...
def upgrade(migrate_engine):
    ...
    pre_meta.tables['assignment'].columns['class_name'].drop()
    post_meta.tables['assignment'].columns['course_name'].create()
...
def downgrade(migrate_engine):
    ...
    pre_meta.bind = migrate_engine
    post_meta.bind = migrate_engine
    pre_meta.tables['assignment'].columns['class_name'].create()
    post_meta.tables['assignment'].columns['course_name'].drop()
